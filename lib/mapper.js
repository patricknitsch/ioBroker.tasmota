...
const { isPowerKey } = require('./datapoints');
...
function parseEnergyObject(obj) {
	...
	const fields = {
		...
		Power: 'sensors.power',
		ApparentPower: 'sensors.apparentPower',
		ReactivePower: 'sensors.reactivePower',
		Factor: 'sensors.factor',
		Today: 'sensors.today',
		Yesterday: 'sensors.yesterday',
		Total: 'sensors.total',
		Period: 'sensors.period',
	};
 ...
 * Known sensor keys are mapped using datapoints; unknown keys go to sensors.{key}.
 *
 * @param {object}  obj
...
function parseSensorObject(obj, prefix) {
	...
	for (const [key, value] of Object.entries(obj)) {
		...
		if (value !== null && typeof value === 'object' && !Array.isArray(value)) {
			...
			// Build state ID: if multiple sensors of same type, prefix with sensor name
