	...
	for (const [key, value] of Object.entries(obj)) {
		...
		if (value !== null && typeof value === 'object' && !Array.isArray(value)) {
			...
			if (dp && dp.channel !== 'controls' && dp.channel !== 'sensors') {
				// Known non-sensor field (e.g. info or energy): route to the declared channel
				// without applying the sensor-device prefix, since these belong to a fixed channel
				const stateKey = key.charAt(0).toUpperCase() + key.slice(1);
				results.push({
					path: `${dp.channel}.${stateKey}`,
					value: coerce(value),
					writable: false,
				});
			} else {
