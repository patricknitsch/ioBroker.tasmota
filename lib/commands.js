...
async function setupCommandManagement(adapter) {
	...
	adapter.processMqttMessage = async function (topic, payload) {
		...
		try {
			...
			// Silently ignore errors in the hook so they never interrupt MQTT
			// message processing.  Log at debug level for troubleshooting.
			adapter.log.debug(`commands hook error: ${hookErr.message}`);
		}
	};
